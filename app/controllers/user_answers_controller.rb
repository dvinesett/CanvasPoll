class UserAnswersController < ApplicationController
  ...
  def create
    questions = params.keys.select{ |i| i =~ /^question_(\d+)$/ }
    questions.each do |question|
      current_answer = Answer.find(params[question])
      current_answer.increment!(:votes)
      # UserAnswer.create(user_id: 0,
